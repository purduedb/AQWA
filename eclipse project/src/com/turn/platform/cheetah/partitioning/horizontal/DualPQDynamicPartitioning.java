...
public class DualPQDynamicPartitioning {

	private long minPartSize = 100000;
	...
	public void archive() {
		...
		ArrayList<Cell> temp = new ArrayList<DualPQDynamicPartitioning.Cell>();
		while (!minHeap.isEmpty()) {
			Cell c = minHeap.remove();
			setCostIncreaseDueToMerge(c);
			temp.add(c);
		}
		for (Cell c : temp) {
			minHeap.add(c);
	...
	private void setCostIncreaseDueToMerge(Cell c) {
		c.costIncreaseDueToMerge = 2 * costEstimator.getSize(c) + costEstimator.getCost(c) - (costEstimator.getCost(c.children[0]) + costEstimator.getCost(c.children[1]));
	}
	...
	public void processNewQueryUpdateStatsOnly(Partition newQuery) {
		...
		for (Cell c : overlappingPartitions) {			
			...
			if (minHeap.contains(c.parent)) {
				setCostIncreaseDueToMerge(c.parent);
	...
	public SplitMergeInfo processNewQuery(Partition newQuery) {
		...
		if (minHeap.peek().costIncreaseDueToMerge < maxHeap.peek().costReductionDueToSplit) {
			...
			setCostIncreaseDueToMerge(toBeMerged.parent);
	...
	public DualPQDynamicPartitioning(CostEstimator costEstimator, int k, int T) {
		this.T= T;
	...
	private void chooseBestCostSplit(Cell parent) {
		...
		if (bestSplits != null) {
			...
			// Avoid small files			
//			if (parent.children[0].getSizeInBytes() < minPartSize || parent.children[1].getSizeInBytes() < minPartSize) {
//				parent.costReductionDueToSplit = 0;
//			}
		}
	}
