...
//			wLoad.add(new QWload(850, 600, 1000));
			...
			for (int i = 0; i < focalXs.length; i++) {
				...
				if (i != 0) {
					wLoad.add(new QWload(focalXs[i], focalYs[i], numBatches * batchSize / 2));
					wLoad.add(new QWload(focalXs[i-1], focalYs[i-1], numBatches * batchSize / 2));
				} else {
	...
	private static void init(JobConf conf, int numFiles, CostEstimator costEstimator) {
		...
		String statsFileName = "/home/aaly/expResults/gradual.csv";		
		try {
