...
public class TimeFading {
	...
	public static void main(String[] args) {
		...
		JobConf conf = new JobConf(mycon, AreaSize.class);
		FileSystem fs = null;
		try {
			...
			for (int w = 10000; w >= 10; w/=10) {
				Common.undoSplits(staticKdPath, aqwaPath);
				aqwaPartitioning = new DynamicPartitioning(costEstimator, numFiles, w);
				aqwaPartitioning.initialPartitions();
				costEstimator.resetQCounts();
				
				// Execute an old workload
				wLoad.clear();
				wLoad.add(new QWload(850, 600, numBatches * batchSize));
				//wLoad.add(new QWload(500, 500, numBatches * batchSize));
				allQueries = QWload.getInterleavedQLoad(wLoad, 10);
				for (int i = 0; i < numBatches; i++)
					AQWAUpdateCountsOnly(conf, fs);

				wLoad.clear();
				wLoad.add(new QWload(600, 600, (numBatches + 10) * batchSize));
				allQueries = QWload.getInterleavedQLoad(wLoad, 10);
				for (int i = 0; i < 10; i++) {					
					AQWAUpdateCountsOnly(conf, fs);
				}
	...
	private static void init(JobConf conf, int numFiles, CostEstimator costEstimator) {
		...
		String statsFileName = "/home/aaly/expResults/fading.csv";		
		try {
		...
		try {
			...
			if (splits.size() > 0) {
				...
				for (SplitMergeInfo splitInfo : splits) {
					System.out.println("Splitting..." + splitInfo.splitParent.getBottom() + ", " + splitInfo.splitParent.getTop() + ", " + splitInfo.splitParent.getLeft() + ", " + splitInfo.splitParent.getRight());					
