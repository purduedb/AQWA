...
public class PartitionsInfo {
  ...
  public PartitionsInfo(JobConf job) {
    ...
    for (String partitionStr : partitionsStrArr) {
      ...
      Partition p = new Partition(Integer.parseInt(coords[0]),
  ...
  public Partition getPartitionID(String [] tokens) {
    ...
    dimensions[0] = 0;
    dimensions[1] = 0;
    List<Partition> partitions = partitionsRTree.searchExclusive(coords,
        dimensions);

    if (partitions.size() > 1) {
      System.out.println(partitions.size());
      System.out.println(partitions.get(0).getBottom() + ","
          + partitions.get(0).getTop() + "," + partitions.get(0).getLeft()
          + "," + partitions.get(0).getRight());
      System.out.println(partitions.get(1).getBottom() + ","
          + partitions.get(1).getTop() + "," + partitions.get(1).getLeft()
          + "," + partitions.get(1).getRight());
