...
public class RTree<T>
{
	...
	public RTree()
	{
		...
		//this(50, 2, 2, SeedPicker.QUADRATIC);
	}
	...
	// This method is added by Ahmed M. Aly to get the overlapping rectangles not including boundary overlap
	public List<T> searchExclusive(double[] coords, double[] dimensions)
	{
		assert (coords.length == numDims);
		assert (dimensions.length == numDims);
		LinkedList<T> results = new LinkedList<T>();
		searchExclusive(coords, dimensions, root, results);
		return results;
	}
	
	// This method is added by Ahmed M. Aly to determine if at least one rectangle overlaps with the query region
	public boolean searchExact(double[] coords, double[] dimensions) {
		assert (coords.length == numDims);
		assert (dimensions.length == numDims);
		
		return searchExact(coords, dimensions, root);		
	}
	
	// This method is added by Ahmed M. Aly to determine if at least one rectangle overlaps with the query region
	private boolean isExactMatch(double[] scoords, double[] sdimensions,
			double[] coords, double[] dimensions) {
		
		for (int i = 0; i < scoords.length; i++)
			if (scoords[i] != coords[i] || sdimensions[i] != dimensions[i])
				return false;
	...
	// This method is added by Ahmed M. Aly to determine if at least one rectangle overlaps with the query region
	private boolean searchExact(double[] coords, double[] dimensions, Node n){
		if (n.leaf)
		{
			for (Node e : n.children)
			{
				if (isExactMatch(coords, dimensions, e.coords, e.dimensions))
				{
					return true;
				}
			}
		...
		else
		{
			for (Node c : n.children)
			{
				...
				{
					return searchExact(coords, dimensions, c);
				}
			}
		}
	...
	private void searchExclusive(double[] coords, double[] dimensions, Node n,
			LinkedList<T> results)
	{
		if (n.leaf)
		{
			for (Node e : n.children)
			{
				if (isExclusiveOverlap(coords, dimensions, e.coords, e.dimensions))
				{
		...
		else
		{
			for (Node c : n.children)
			{
				if (isExclusiveOverlap(coords, dimensions, c.coords, c.dimensions))
				{
					searchExclusive(coords, dimensions, c, results);
				}
			}
		}
	}
	...
	private Node findLeaf(Node n, double[] coords, double[] dimensions, T entry)
	{
		...
		else
		{
			for (Node c : n.children)
			{
				...
				if (isExclusiveOverlap(c.coords, c.dimensions, coords, dimensions))
				{	
	...
	// This method is added by Ahmed M. Aly to get the overlapping rectangles not including boundry overlap
	private boolean isExclusiveOverlap(double[] scoords, double[] sdimensions,
			double[] coords, double[] dimensions)
	{
		for (int i = 0; i < scoords.length; i++)
		{
	...
	public class Node
	{
		...
		public final LinkedList<Node> children;
		public final boolean leaf;

		public double distance; // could be mindist or maxdist.
		...
		public boolean equals(Object node) {
			if (((Node) node).coords.length != this.coords.length)
				return false;
			for (int i = 0; i < ((Node)node).coords.length; i++) {
				if (((Node) node).coords[i] != this.coords[i])
					return false;
				if (((Node) node).dimensions[i] != this.dimensions[i])
					return false;
			}
			return true;
		}
	...
	// The following is added by Ahmed M. Aly
